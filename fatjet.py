...
class FatJet(object):
    def __init__(self,tower=None,algorithm="antikt",r=1.2,pt_min=200.,verbose=False):
    ...
    def ConstructVector(self,fatjet,maximum_particles=None):
        ...
        elif type(fatjet[0])==TLorentzVector: fatjet=ru.GetNumpy(fatjet,format="lhc",observable_first=False)
        vectors=[]
    ...
    def RemoveElectron(self,lepton):
        ...
        if self.Verbose: print (lepton,self.fatjets)
        electron_eta_phi=[]
        for item in lepton:
        ...
        for i in range(len(self.fatjets)):
            for item in electron_eta_phi:
                R=np.sqrt((self.fatjets[i].eta-item[0])**2+(self.fatjets[i].phi-item[1])**2)
                if R<self.R:
                    if i not in indices:indices.append(i)
        FatJets=[]
        for i in range(len(self.fatjets)):
            if i not in indices: FatJets.append(self.fatjets[i])
        self.fatjets=FatJets
        if self.Verbose: print (self.fatjets)
        return self.fatjets
    def Get(self):
        '''get list of fatjet in PseudoJet class'''
        if type(self.Tower[0])==np.ndarray: temp=np.concatenate((self.Tower,np.zeros((len(self.Tower),1))),axis=1)
        else: temp=ru.GetNumpy(self.Tower,format="lhc",observable_first=False)
        vectors=[]
        for item in temp:
            vectors.append(np.array((item[0],item[1],item[2],item[3]),dtype=[('pT', 'f8'), ('eta', 'f8'), ('phi', 'f8'), ('mass', 'f8')]))
    ...
    def GetConstituents(self,fatjets,format="root"):
        ...
        if format=="image":
            for item in fatjets: 
                return_array.append(np.swapaxes(np.array([[particle.eta,particle.phi,particle.pt] for particle in item.constituents()],dtype="float64"),0,1))
            return return_array
        for item in fatjets:
            if format=="root":return_array.append(ru.GetTLorentzVector(item,format="fatjet"))
            else: return_array.append(np.array([item.eta,item.phi,item.pt],dtype="float64"))
        return np.array(return_array) 
    #@methods
    def Recluster(self,fatjet,r=0.4,dcut=0.5,algorithm="CA",pt_min=None,subjets=None,return_val="inc_jets",maximum_particles=None):
        ...
        if return_val=="sequence": 
            ...
            if pt_min is None: pt_min=0.03*np.sum(fatjet).Pt() 
...
def Print(fatjet,name=None,format="lhc",constituents=False):
    ...
    if not constituents: 
        if format=="lhc": print (f"    Eta: {fatjet.eta:20.16f}        Phi: {fatjet.phi:20.16f}        Pt : {fatjet.pt:20.16f}        Mass: {fatjet.mass:20.16f}")
        else: print (f"    Px: {fatjet.px:20.16f}        Py: {fatjet.py:20.16f}        Pz : {fatjet.pz:20.16f}        E: {fatjet.e:20.16f}")
    else:
        print ("Constituents of array: ")
        for item in fatjet.constituents(): 
            if format=="lhc": print (f"    Eta: {item.eta:20.16f}        Phi: {item.phi:20.16f}        Pt : {item.pt:20.16f}        Mass: {item.mass:20.16f}")
            else: print (f"    Px : {item.px:20.16f}        Py: {item.py:20.16f}        Pz: {item.pz:20.16f}        E : {item.E:20.16f}")
    return
