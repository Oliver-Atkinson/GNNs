...
def GetTLorentzVector(array,format="lhc",particle="visible"):
    ...
    if len(array.shape)==1:
        ...
        if format=="lhc":
            ...
            vec.SetPxPyPzE(array[0],array[1],array[2],array[3])
        return vec
    else:
        ...
        return np.array(vec)
def GetNumpy(vectors,format="image",observable_first=False,exclude_mass=False,diff=None,include_del_r=False):
    ...
    if format=="image":
        ...
        if type(vectors)==TLorentzVector: 
            if exclude_mass:return np.array([vectors.Pt(),vectors.Eta(),vectors.Phi()],dtype="float64") 
            else: return np.array([vectors.Pt(),vectors.Eta(),vectors.Phi(),vectors.M()],dtype="float64")
        else:
            if exclude_mass:
                if include_del_r: return_array=np.zeros((len(vectors),4),dtype="float64") 
                else: return_array=np.zeros((len(vectors),3),dtype="float64")
                for i in range(len(vectors)):
                    if vectors[i].M()==0 and vectors[i].P()==0: continue
                    if diff is None: return_array[i]=np.array([vectors[i].Pt(),vectors[i].Eta(),vectors[i].Phi()],dtype="float64")
                    else: 
                        if include_del_r:
                            return_array[i]=np.array([vectors[i].Pt(),diff.Eta()-vectors[i].Eta(),
                                                      diff.DeltaPhi(vectors[i]),diff.DeltaR(vectors[i])],dtype="float64")
                        else: return_array[i]=np.array([vectors[i].Pt(),diff.Eta()-vectors[i].Eta(),diff.DeltaPhi(vectors[i])],dtype="float64")
            else:
...
def Sort(array,attribute="pt",order="desc",return_indices=False):
    ...
    if order=="desc": indices=np.flip(indices)
    if return_indices: return array[indices],indices
